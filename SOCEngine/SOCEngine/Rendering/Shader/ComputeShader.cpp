...
{	
	...
	dx.GetContext()->CSSetShaderResources(static_cast<uint32>(bind), 1, &raw);
}
...
{	
	...
	dx.GetContext()->CSSetSamplers(static_cast<uint32>(bind), 1, &samplerState);
}
...
void ComputeShader::BindConstBuffer(DirectX& dx, ConstBufferBindIndex bind, const ConstBuffer& cb)
{
	...
	dx.GetContext()->CSSetConstantBuffers(static_cast<uint32>(bind), 1, &buf);
}

void ComputeShader::BindUnorderedAccessView(DirectX& dx, UAVBindIndex bind, const UnorderedAccessView& uav, const uint32* initialCounts)
{
	ID3D11UnorderedAccessView* view = const_cast<UnorderedAccessView&>(uav).GetRaw();
	dx.GetContext()->CSSetUnorderedAccessViews(static_cast<uint32>(bind), 1, &view, initialCounts);
}

void ComputeShader::UnBindSamplerState(DirectX& dx, SamplerStateBindIndex bind)
{
	ID3D11SamplerState* sampler = nullptr;
	dx.GetContext()->CSSetSamplers(static_cast<uint32>(bind), 1, &sampler);
}
...
void ComputeShader::UnBindShaderResourceView(DirectX& dx, TextureBindIndex bind)
{
	...
	dx.GetContext()->CSSetShaderResources(static_cast<uint32>(bind), 1, &srv);
}

void ComputeShader::UnBindUnorderedAccessView(DirectX& dx, UAVBindIndex bind)
{
	ID3D11UnorderedAccessView* view = nullptr;
	dx.GetContext()->CSSetUnorderedAccessViews(static_cast<uint32>(bind), 1, &view, nullptr);
}
